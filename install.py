...
def info(msg: str) -> None:
    print(f"\033[36m[md-cloud]\033[0m {msg}")
...
def step_venv() -> Path:
    ...
    info("安装/更新 md-cloud 包（非 editable，兼容 Python 3.14+）")
...
def step_ping(py: Path, creds: dict[str, str]) -> None:
    ...
    code = (
        "from md_cloud.auth import ensure_access_token;"
...
def step_mcp_config(py: Path, creds: dict[str, str]) -> None:
    ...
    if mode in ("1", "3"):
        ...
        existing.setdefault("mcpServers", {})["md-cloud"] = server_entry
        MCP_JSON.write_text(
    ...
    if mode in ("2", "3"):
        ...
        if not claude_bin:
            ...
            # 先尝试移除已存在的同名条目，避免冲突
            run(
                [claude_bin, "mcp", "remove", "md-cloud", "--scope", "user"],
                check=False,
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL,
            )
            cmd = [
                claude_bin, "mcp", "add", "md-cloud",
                "--scope", "user",
                "-e", f"MD_ACCOUNT_ID={creds['MD_ACCOUNT_ID']}",
                "-e", f"MD_KEY={creds['MD_KEY']}",
                "--", str(py), "-m", "md_cloud.server",
            ]
            try:
                run(cmd)
...
def step_done() -> None:
    info("步骤 5/5：完成")
    ok("md-cloud 安装完毕。重启 Claude Code 即可使用。")
    print("\n试试在 Claude Code 里说：")
    print("  · 「帮我看看最近的公司动态」")

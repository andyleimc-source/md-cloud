...
<html><head><meta charset="utf-8"><title>md-cloud 授权成功</title></head>
...
<html><head><meta charset="utf-8"><title>md-cloud 授权失败</title></head>
<body style="font-family:system-ui;max-width:560px;margin:80px auto;padding:0 24px;color:#222">
<h2>❌ 授权失败</h2>
...
def run_auth_flow(project_root: Path | None = None) -> dict[str, str]:
    ...
    try:
        ...
        raise RuntimeError(
            f"端口 {port} 已被占用。用 MD_CALLBACK_PORT=xxxx mdcloud-auth 换端口"
    ...
    if not res:
        raise RuntimeError("等待授权超时（5 分钟），请重新运行 mdcloud-auth。")
    if "error" in res:
